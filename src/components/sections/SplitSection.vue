  ...
  <section class="py-16 px-6 bg-(--color-bg)">
    <div class="max-w-5xl mx-auto flex flex-col md:flex-row gap-10 items-center" :class="section?.imageRight ? '' : 'md:flex-row-reverse'">
      <div class="flex-1 min-w-0">
        <p v-if="section?.eyebrow" class="text-sm font-semibold uppercase tracking-widest text-(--color-primary) mb-3">{{ section.eyebrow }}</p>
        <h2 v-if="section?.heading" class="text-3xl font-bold text-(--color-text) mb-4 leading-tight">{{ section.heading }}</h2>
        <p v-if="section?.body" class="text-(--color-text-secondary) text-base leading-relaxed mb-6">{{ section.body }}</p>
        <ul v-if="bulletPoints.length" class="space-y-2 mb-8">
          <li v-for="(f, i) in bulletPoints" :key="i" class="flex items-start gap-2 text-sm text-(--color-text)">
            <span class="font-bold mt-0.5 text-(--color-primary)">✓</span> {{ typeof f === 'string' ? f : f.title }}
          </li>
        </ul>
