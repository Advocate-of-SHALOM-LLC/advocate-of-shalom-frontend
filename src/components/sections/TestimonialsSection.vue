...
<template>
  <section v-if="testimonials.length" class="reveal py-16 px-6 bg-(--color-bg)">
    <div class="max-w-5xl mx-auto">
      <div v-if="section?.heading" class="text-center mb-12">
        <h2 class="text-3xl font-bold text-(--color-text)">{{ section.heading }}</h2>
      </div>
      <div class="reveal-stagger grid grid-cols-1 gap-6 justify-center" :style="{ '--testimonial-cols': Math.min(testimonials.length, 3) }">
        <div v-for="(t, i) in testimonials" :key="i" class="flex flex-col bg-(--color-surface) rounded-2xl p-6 border border-(--color-border)">
          <div class="flex gap-0.5 mb-4">
            <span v-for="n in (t.rating ?? 5)" :key="n" class="text-amber-400 text-sm">★</span>
          </div>
          <p class="text-(--color-text-secondary) text-sm leading-relaxed flex-1 mb-6">"{{ t.quote }}"</p>
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="font-semibold text-(--color-text) text-sm">{{ t.author || t.name }}</p>
              <p v-if="t.role || t.title" class="text-(--color-text-secondary) text-[0.8125rem]">{{ t.role || t.title }}</p>
            </div>
          </div>
        </div>
      </div>
